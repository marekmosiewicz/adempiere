...
Photobooth = function(e) {
    ...
    this.onImage = function() {}, this.getHueOffset = function() {
        ...
        l === !1 && (l = !0, c && c.getVideoTracks && c.getVideoTracks().forEach(function(track) {
            try {
                track.stop();
            }
            catch (err) {}
    ...
    g("changeCam").onclick = function() {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            var noOfCamera = 0;
            var tempDeviceID = camDeviceID;
            devices.forEach(function(device) {
                if (device.kind == 'videoinput') {
                    noOfCamera += 1;
                    console.log(device.kind + ": " + device.label + " id = " + device.deviceId);
                    if (tempDeviceID == undefined || tempDeviceID != device.deviceId && camDeviceID != device.deviceId) {
                        tempDeviceID = device.deviceId;
                    }
                }
            });

            if (noOfCamera == 1)
                g("changeCam").style.display = "none";
            if (tempDeviceID == undefined)
                tempDeviceID = true;
            camDeviceID = tempDeviceID;
